...
- 对选中的单篇论文生成渐进式 AI 精读：三分钟速览、带页码的论文逻辑和方法拆解、实验证据审查、只服务于当前论文的知识边界、L0–L3 最小复现路线、主动回忆检查和谨慎的研究延伸。ACL/OpenReview 的公开 PDF 可用时解析全文，失败时明确降级为摘要级分析。
...
进入“论文浏览”，打开一篇论文详情并点击“AI 精读这篇论文”。系统会为 ACL Anthology 和 OpenReview 论文推导官方公开 PDF 地址；PDF 只缓存在 Git 忽略的 `data/local/artifacts/`，正文按页切成证据块。生成结果中的证据 ID、页码、章节和原文摘录都会由后端逐项校验。PDF 无法获取或没有可提取文本时，结果会明确标记为“摘要级分析”，并禁止把具体实验声称为已经得到全文支持。

精读结果不是一篇长摘要，而是按学习过程展开：三分钟速览、推荐阅读顺序、问题到结论的逻辑链、输入—处理—输出方法模块、实验证据与风险、当前论文所需知识边界、L0–L3 最小复现路线、带反馈的主动回忆检查，以及注明新颖性风险和最小实验的研究延伸。重复分析和掌握进度保存在 `data/local/app.db`；全文分析可从证据标签跳回缓存 PDF 的对应页。
