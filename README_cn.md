...
- 为选定方向生成经过结构校验的知识树、7/30/90 天计划、锚点论文、L0–L4 复现阶梯和最小研究假设。
...
云端返回必须通过本地 Pydantic 合同：知识依赖无环、计划日期连续、锚点论文 ID 来自当前证据、任务有可度量产物与验收标准、复现阶梯严格覆盖 L0–L4。格式错误会成为失败任务，不会保存成可用计划。
